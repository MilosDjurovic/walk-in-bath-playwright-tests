...
export class LandingFormPage {
	...
	constructor(page: Page) {
		...
		this.thankYouHeading = this.page.getByRole('heading', { name: 'Thank you!' });
	}
	...
	private buildFormLocators(containerSelector: string): FormLocators {
		...
		return {
			...
			outOfAreaEmailInput: container.getByRole('textbox', { name: 'Email Address' }),
			...
			interestOption: (optionText: string) => container.getByText(optionText, { exact: true }),
			propertyTypeOption: (optionText: string) => container.getByText(optionText, { exact: true }),
			nameInput: container.getByRole('textbox', { name: 'Enter Your Name' }),
			emailInput: container.getByRole('textbox', { name: 'Enter Your Email' }),
			goToEstimateButton: container.getByRole('button', { name: 'Go To Estimate' }),
			phoneInput: container.getByRole('textbox', { name: '(XXX)XXX-XXXX' }),
			submitRequestButton: container.getByRole('button', { name: 'Submit Your Request' }),
			zipErrorMessage: container.getByText(/Enter your ZIP code\.|Wrong ZIP code\./),
		};
	}
}
